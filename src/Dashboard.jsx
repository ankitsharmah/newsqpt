...
const Dashboard = () => {
  ...
  const fetchSpreadsheets = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:9000/api/api/spreadsheets');
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      setUploading(true);
      const response = await axios.post('http://localhost:9000/api/api/upload', formData, {
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:9000/api/api/spreadsheets/${selectedSpreadsheet._id}`);
